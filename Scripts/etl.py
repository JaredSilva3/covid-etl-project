...
DATABASE_URL = "postgresql://<username>:<password>@<host>:<port>/<database_name>"  # Replace placeholders with your actual credentials
...
def transform_data(covid_full_data, owid_data):
    ...
    cases_and_deaths_owid = owid_data[
        ["date", "location", "new_cases", "new_deaths", "total_cases", "total_deaths"]
    ].copy()
    cases_and_deaths_owid["date"] = pd.to_datetime(cases_and_deaths_owid["date"])

    # Death rates from OWID data
    death_rates = owid_data[
        ["continent", "location", "date", "total_cases", "total_deaths"]
    ].copy()
    death_rates = death_rates[
        death_rates[["total_cases", "total_deaths"]].notnull().all(axis=1)
    ]
    death_rates["death_rate"] = (
        death_rates["total_deaths"] / death_rates["total_cases"]
    ) * 100
    death_rates["date"] = pd.to_datetime(death_rates["date"])

    # Regional deaths comparison from OWID data
    regional_deaths = owid_data[
        ["continent", "location", "date", "new_deaths", "total_deaths"]
    ].copy()
